...
output = subprocess.check_output('ls {} | grep -v .cpp | grep -v .xml | grep -v .md'.format(treeanalysis_bin_dir), shell=True, **popen_extra_args).rstrip('\n')
...
retcode = subprocess.call(checkIfAnalysisIsRegistered, shell=True, **popen_extra_args)
if (not retcode == 0):
