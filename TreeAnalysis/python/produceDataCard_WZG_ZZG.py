...
from VZGsamplesByRegion import getVZGSamplesByRegion
...
__builtin_config__ = {
    ...
    'regions': {
        'SR2P': {
            'processes': {'WZG': -1, 'ZZG': -1, 'TTTo2L2Nu':-1, 'DYJetsToLL_M50':-1, 'ZGToLLG':-1, 'FSR':-1},
        ...
        'SR2PFJ': {
            'processes': {'WZG': -1, 'ZZG': -1, 'TTTo2L2Nu':-1, 'DYJetsToLL_M50':-1, 'ZGToLLG':-1},
...
def get_sample_group_QCDscale(sample):
    if  (sample.startswith(('ZZGTo4LG','WZGTo3LNuG','ZZGTo2L2jG','WZGTo2L2jG', 'VZG', 'WZG','ZZG' ))):
        return 'VVgamma'
...
def main(args):
    ...
    if(args.path is None):
        ...
        args.path = os.path.join('/eos/home-c/ctarrico/Frameworks/CMSSW_10_6_26/src/VVXAnalysis/TreeAnalysis', args.localpath)
    ...
    df_syst=df_syst.reindex( sorted(df_syst.columns, key = lambda x: samples_to_idx[x] if x != "type" else -1000) , axis=1)
