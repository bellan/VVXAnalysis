...
def GetError(region,i):
    ...
    for d in typeofsamples:
        filesCR2P2F[d["sample"]] = ROOT.TFile(inputdirCR2P2F+d["sample"]+".root")
        filesCR3P1F[d["sample"]] = ROOT.TFile(inputdirCR3P1F+d["sample"]+".root")

    hvar=ROOT.TH1F()
    First = 1
    for d in typeofsamples:
        hv31 = filesCR3P1F[d["sample"]].Get("ZZMass_FRVar")
       # print d["sample"],"INTEGRAL  = ", h.Integral(), "N BINS", h.GetNbinsX()
        if hv31 == None:
            continue
        if First:
            hvar=copy.deepcopy(hv31)
            hv22=filesCR2P2F[d["sample"]].Get("ZZMass_FRVar")
            #print "varval1 = ", hv22.GetBinContent(i)
            First=0
            if hv22==None:
                continue
            hvar.Add(hv22)
        ...
        hv22=filesCR2P2F[d["sample"]].Get("ZZMass_FRVar")
        print "varval2 = ", hv22.GetBinContent(i)
        if hv22==None:
            continue
        hvar.Add(hv22)
...
def GetFakeRate(region, method): 
    ...
    for s in typeofsamples:
        ...
        hv31 = filesCR3P1F[s["sample"]].Get("ZZMass")
        if h31==None:
