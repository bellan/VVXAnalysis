  ...
  */
  ...
  for(int iRegion=0; iRegion<regions.size(); iRegion++){
    for (int iVars=0; iVars<varNames.size(); ++iVars){
      ...
      if(varName=="j0_Girth") varLabel="leading jet girth";
      if(varName=="j1_Girth") varLabel="sub-leading jet girth";
      if(varName=="dRJ0Gamma_") varLabel="#Delta R_{j0#gamma}";
      if(varName=="dRJ1Gamma_") varLabel="#Delta R_{j1#gamma}";
      //      if(varName=="H_T\ ") varLabel="VZ#gamma cand. tot. p_{T} [GeV]";
      //      if(varName=="H_T")   varLabel="VZ#gamma cand. tot. p_{T} [GeV]";
      if(varName=="recoVPt_") varLabel="V had. cand. p_{T} [GeV]";
      if(varName=="recoZPt_") varLabel="Z (dilepton) cand. p_{T} [GeV]";
      if(varName=="VZGpT"||varName=="pre-Rwgt_VZGpT") varLabel="VZ#gamma cand. tot. p_{T} [GeV]";
      if(varName=="VZpT" ||varName=="pre-Rwgt_VZpT" ) varLabel=      "VZ cand. tot. p_{T} [GeV]";
      if(varName=="ZGpT" ||varName=="pre-Rwgt_ZGpT" ) varLabel= "Z#gamma cand. tot. p_{T} [GeV]";
      if(varName=="VGpT" ||varName=="pre-Rwgt_VGpT" ) varLabel= "V#gamma cand. tot. p_{T} [GeV]";
	...
	//      }
      ...
      MCHist->SetFillColor(kBlack);// Spessore del contorno aumentato
      MCHist->SetLineColor(kBlack);// Spessore del contorno aumentato
      MCHist->SetLineWidth(2);

      TH1F *MCContour = (TH1F*)MCHist->Clone("MCContour");
      MCContour->SetFillStyle(0);
      //      MCContour->SetMarkerStyle(1);
      MCContour->SetFillColor(kBlack);
      MCContour->SetLineColor(kBlack);// Spessore del contorno aumentato
      MCContour->SetLineWidth(1);

      
      for (int i = 1; i <= DYNonPromptHist->GetNbinsX(); i++) {

	double dy_mc     = DYNonPromptHist->GetBinContent(i);
	double dy_mc_err = DYNonPromptHist->GetBinError(i);
	double dy_rel_mc_err = dy_mc > 0 ? dy_mc_err/dy_mc : 0.0;
	dy_rel_mc_err = TMath::Sqrt(dy_rel_mc_err*dy_rel_mc_err+dy_rel_rew_err*dy_rel_rew_err);
	dy_mc_err=dy_rel_mc_err*dy_mc;
	DYNonPromptHist->SetBinError(i,dy_mc_err);
      }
      for (int i = 1; i <= ZGPromptHist->GetNbinsX(); i++) {

	double zg_mc     = ZGPromptHist->GetBinContent(i);
	double zg_mc_err = ZGPromptHist->GetBinError(i);
	double zg_rel_mc_err = zg_mc > 0 ? zg_mc_err/zg_mc : 0.0;
	zg_rel_mc_err = TMath::Sqrt(zg_rel_mc_err*zg_rel_mc_err+zg_rel_rew_err*zg_rel_rew_err);
	zg_mc_err=zg_rel_mc_err*zg_mc;
	ZGPromptHist->SetBinError(i,zg_mc_err);	
      }

      TH1F *MCrewErrHist = (TH1F*)ZGPromptHist->Clone("MCrewErrHist");
      //      if(regions[iRegion]=="CR2P_1VL"){
      //	MCrewErrHist->Add(DYPromptHist);
      MCrewErrHist->Add(DYNonPromptHist);
	//      }
      MCrewErrHist->Add(signalHist);
      //MCrewErrHist->Add(ZGPromptHist);
      //MCrewErrHist->Add(ZGNonPromptHist);
      
      MCrewErrHist->Add(VZFSRHist);
      MCrewErrHist->Add(TTTo2L2NuHist);
      //	MCrewErrHist->Add(ttXHist);
      //	MCrewErrHist->Add(tZqHist);

      //      MCrewErrHist->SetFillStyle(0);
      MCrewErrHist->SetFillStyle(3005);
      MCrewErrHist->SetFillColor(kAzure-3);// Spessore del contorno aumentato
      MCrewErrHist->SetLineColor(kAzure-3);// Spessore del contorno aumentato
      MCrewErrHist->SetLineWidth(2);

      TH1F *MCrewErrContour = (TH1F*)MCrewErrHist->Clone("MCrewErrContour");
      MCrewErrContour->SetFillStyle(0);
      MCrewErrContour->SetFillColor(kAzure);// Spessore del contorno aumentato
      MCrewErrContour->SetLineColor(kAzure);// Spessore del contorno aumentato
      MCrewErrContour->SetLineWidth(1);
	...
	//	else stack->SetMaximum(aggrBin*10000);
      ...
      */
      ...
      MCHist->Draw("sameE2");
      MCrewErrHist->Draw("sameE2");
      MCrewErrContour->Draw("sameE2");
      MCContour->Draw("sameE2");
      ...
      legend->AddEntry(MCHist, "stat. unc.", "f");
      legend->AddEntry(MCrewErrHist, "stat. #oplus rew. unc.", "f");
      ...
      TH1F *relMCerrHist = (TH1F*)MCHist->Clone("relMCerrHist");

      for (int i = 1; i <= MCHist->GetNbinsX(); i++) {

	double mc     = MCHist->GetBinContent(i);
	double mc_err = MCHist->GetBinError(i);
	double rel_mc_err = mc > 0 ? mc_err/mc : 0.0;
	//	rel_mc_err = TMath::Sqrt(rel_mc_err*rel_mc_err+0.3*0.3);
	relMCerrHist->SetBinContent(i,1.0);
	relMCerrHist->SetBinError(i,rel_mc_err);
	
      }

      //ratioPad->cd()
      relMCerrHist->SetFillStyle(3004);
      relMCerrHist->SetFillColor(kBlack);// Spessore del contorno aumentato
      relMCerrHist->SetLineColor(kBlack);// Spessore del contorno aumentato
      relMCerrHist->SetLineWidth(2);

      relMCerrHist->Draw("sameE2");

      TH1F *relMCerrContour = (TH1F*)relMCerrHist->Clone("relMCerrContour");
      relMCerrContour->SetFillStyle(0);
      relMCerrContour->SetLineColor(kBlack);// Spessore del contorno aumentato
      relMCerrContour->SetLineWidth(1);

      relMCerrContour->Draw("sameE2");

      
      TH1F *relRWGerrHist = (TH1F*)MCrewErrHist->Clone("relRWGerrHist");

      for (int i = 1; i <= MCrewErrHist->GetNbinsX(); i++) {

	double mc     = MCrewErrHist->GetBinContent(i);
	double mc_err = MCrewErrHist->GetBinError(i);
	double rel_mc_err = mc > 0 ? mc_err/mc : 0.0;
	//	rel_mc_err = TMath::Sqrt(rel_mc_err*rel_mc_err+0.3*0.3);
	relRWGerrHist->SetBinContent(i,1.0);
	relRWGerrHist->SetBinError(i,rel_mc_err);
	
      }

      //ratioPad->cd();
      relRWGerrHist->SetFillStyle(3005);
      relRWGerrHist->SetLineColor(kAzure-3);// Spessore del contorno aumentato
      relRWGerrHist->SetLineWidth(2);
      
      relRWGerrHist->SetTitle("");
      relRWGerrHist->Draw("sameE2");

      TH1F *relRWGerrContour = (TH1F*)relRWGerrHist->Clone("relRWGerrContour");
      relRWGerrContour->SetFillStyle(0);
      relRWGerrContour->SetFillColor(kAzure);// Spessore del contorno aumentato
      relRWGerrContour->SetLineColor(kAzure);// Spessore del contorno aumentato
      relRWGerrContour->SetLineWidth(1);

      relRWGerrContour->Draw("sameE2");
      ...
      if (isLogScale) {
	canvas->SaveAs("../DistributionPlots/Run2/LogScale/results_withRewUnc_ForANv5plus_"+varName+"_"+regions[iRegion]+".png");
	canvas->SaveAs("../DistributionPlots/Run2/LogScale/results_withRewUnc_ForANv5plus_"+varName+"_"+regions[iRegion]+".pdf");
      }/*
      else {
  ...
  for (int cutNb = 3; cutNb < 5; cutNb++){
	...
	stack->SetMinimum(1.1);
      ...
      if (isLogScale){
	canvas->SaveAs("../DistributionPlots/Run2/LogScale/resultsForThesisPlots_BLINDED_"+varName+cutNb+"_.png");
	canvas->SaveAs("../DistributionPlots/Run2/LogScale/resultsForThesisPlots_BLINDED_"+varName+cutNb+"_.pdf");
      }
      /*	
      if (isLogScale){
