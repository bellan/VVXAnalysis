...
void unblindedCMSDataMC()
{
  ...
  for(int i=0; i<1; i++){
    ...
    */
	...
	if(varName=="recoVDaughter0Pt_") varLabel="leading jet p_{T} [GeV]";
	if(varName=="recoVDaughter1Pt_") varLabel="sub-leading jet p_{T} [GeV]";
	...
	if(varName=="dRJ0Gamma_") varLabel="#Delta R_{j_{0}#gamma}";
	if(varName=="dRJ1Gamma_") varLabel="#Delta R_{j_{1}#gamma}";
	if(varName=="dPhiZG_") varLabel="#Delta #phi_{Z#gamma}";
	if(varName=="ptGamma_") varLabel="p_{T}^{#gamma} [GeV]";
	if(varName=="H_T") varLabel="H_{T} [GeV]";
	if(varName=="PhotonMVAID_") varLabel="#gamma MVA ID";
	if(varName=="PhotonMVAIDBinned_") varLabel="#gamma MVA ID";
	...
	*/
	...
	if (isLogScale){
	  canvas->SaveAs("../DistributionPlots/"+year+"/LogScale/"+resDir+"_"+varName+"_"+regions[iRegion]+".png");
	  canvas->SaveAs("../DistributionPlots/"+year+"/LogScale/"+resDir+"_"+varName+"_"+regions[iRegion]+".pdf");
	}
	else {
	  canvas->SaveAs("../DistributionPlots/"+year+"/LogScale/"+resDir+"_"+varName+"_"+regions[iRegion]+".png");
	  canvas->SaveAs("../DistributionPlots/"+year+"/LogScale/"+resDir+"_"+varName+"_"+regions[iRegion]+".pdf");
	...
	for (int iVars=0; iVars<varNames.size(); ++iVars){
	  ...
	  */
	  ...
	  if (isLogScale){
	    canvas->SaveAs("../DistributionPlots/"+year+"/LogScale/"+resDir+"_"+varName+"_SR"+cutNb+".png");
	    canvas->SaveAs("../DistributionPlots/"+year+"/LogScale/"+resDir+"_"+varName+"_SR"+cutNb+".pdf");
	  }
	  else {
	    canvas->SaveAs("../DistributionPlots/"+year+"/LogScale/"+resDir+"_"+varName+"_SR"+cutNb+".png");
	    canvas->SaveAs("../DistributionPlots/"+year+"/LogScale/"+resDir+"_"+varName+"_SR"+cutNb+".pdf");
