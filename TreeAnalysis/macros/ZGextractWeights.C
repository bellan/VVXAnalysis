...
void ZGextractWeights() {
  gStyle->SetOptStat(0);
    ...
    for (int i = 0; i < years.size(); i++) {
        ...
        TH1F* hNum = (TH1F*)DataFile->Get("REWGT-ZG_numDATA");
        TH1F* hDen = (TH1F*)ZGFile->Get("REWGT-ZG_ZG-GEN");
        TH1F* hDY = (TH1F*)DYFile->Get("REWGT-ZG_else-GEN");
        TH1F* hTT = (TH1F*)TTFile->Get("REWGT-ZG_else-GEN");
	hNum->Add(hDY, -1);
	hNum->Add(hTT, -1);
	cout <<"{ ";
