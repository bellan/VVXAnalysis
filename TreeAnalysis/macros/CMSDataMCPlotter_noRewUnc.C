  ...
  */
  ...
  for(int iRegion=0; iRegion<regions.size(); iRegion++){
    for (int iVars=0; iVars<varNames.size(); ++iVars){
      ...
      if(varName=="PRE-RWGT_recoZPt_") varLabel="before reweighting: Z (dilepton) cand. p_{T} [GeV]";
      if(varName=="POST-RWGT_recoZPt_") varLabel="Z (dilepton) cand. p_{T} [GeV]";
	...
	relMCerrHist->SetBinError(i,rel_mc_err);
      ...
      if (isLogScale) {
	canvas->SaveAs("../DistributionPlots/Run2/LogScale/resultsForANv5plus_"+varName+"_"+regions[iRegion]+".png");
	canvas->SaveAs("../DistributionPlots/Run2/LogScale/resultsForANv5plus_"+varName+"_"+regions[iRegion]+".pdf");
      }/*
      else {
