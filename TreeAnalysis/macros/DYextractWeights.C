...
TString resDir = "resultsPreZXRwgt";
const std::vector<TString> years = {"2016preVFP", "2016postVFP", "2017", "2018", "Run2"};

void DYextractWeights() {
  gStyle->SetOptStat(0);
    ...
    Color_t colors[] = {kRed+1, kSpring+3, kBlue, kViolet, kBlack  };
    Int_t markerStyles[] = {21,22,23,27,20};
    ...
    for (int i = 0; i < years.size(); i++) {
        ...
        TH1F* hNum = (TH1F*)DataFile->Get("REWGT-DY_numDATA");
        TH1F* hDen = (TH1F*)DYFile->Get("REWGT-DY_DY-GEN");
        TH1F* hZG = (TH1F*)ZGFile->Get("REWGT-DY_else-GEN");
        TH1F* hTT = (TH1F*)TTFile->Get("REWGT-DY_else-GEN");
	hNum->Add(hZG, -1);
	hNum->Add(hTT, -1);
	cout <<"{ ";
    ...
    */
	...
	hEffs[i]->GetYaxis()->SetTitleSize(0.035); // Imposta la dimensione del titolo per l'asse Y
      ...
      hEffs[i]->GetXaxis()->SetTitle("gen. Z p_{T} [GeV]");
    ...
    paveTLeft->AddText("CMS #bf{Preliminary}");
