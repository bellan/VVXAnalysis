...
void ResponseMatrix_builder(string var = "Mass", string dataset = "01", string finalstate = "4e", int xs_qq = 0, int xs_gg =0, bool weight = 0)//bool eta = false)
{
  ...
  TFile *ZZTo4mu_r = new TFile("../results/ZZRecoAnalyzer_SR/ZZTo4mu.root");
  TFile *ZZTo4e_r = new TFile("../results/ZZRecoAnalyzer_SR/ZZTo4e.root");
  TFile *ZZTo2e2mu_r = new TFile("../results/ZZRecoAnalyzer_SR/ZZTo2e2mu.root");
  ...
  TFile *ZZTo4mu_g = new TFile("../results/ZZMCAnalyzer_MC/ZZTo4mu.root");
  TFile *ZZTo4e_g = new TFile("../results/ZZMCAnalyzer_MC/ZZTo4e.root");
  TFile *ZZTo2e2mu_g = new TFile("../results/ZZMCAnalyzer_MC/ZZTo2e2mu.root");
 TFile *ZZTo2e2muJJ_g = new TFile("../results/ZZMCAnalyzer_MC/ZZTo2e2muJJ_SMHContinInterf_H125.6.root");
  ...
  if(xs_qq == 0) { 
    h_Resmat_qqTot->Scale(1);
    h_qq4mu->Scale(1);
    h_qq4mu_gen->Scale(1); 
  } 
  else if(xs_qq == 1) {
    h_Resmat_qqTot->Scale(1+unc_qq); 
    h_qq4mu->Scale(1+unc_qq); 
    h_qq4mu_gen->Scale(1+unc_qq);
  }
  else if(xs_qq == -1){
    h_Resmat_qqTot->Scale(1-unc_qq); 
    h_qq4mu->Scale(1-unc_qq); 
  ...
  totalint = h_Resmat_qqTot->Integral();

  h_Resmat_normTot = (TH2*)h_Resmat_qqTot->Clone("h_Resmat_qqTot");
...
void PlotMatrix(string var = "Mass", string fs = "4e", string dataset = "01", string unc = "st",bool weight = 0){
  ...
  string finalstate;
  double max = 0;  
  string fileName = var+"_test/matrices_Pow.root";
  string fileName_W = var+"_test/weightedMatrices_Pow.root";
  ...
  if(var =="Mass"){
    ...
    xAxis = "reco m_{"+finalstate+"}";
    yAxis = "gen m_{"+finalstate+"}";
