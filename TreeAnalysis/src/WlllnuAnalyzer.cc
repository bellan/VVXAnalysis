...
void WlllnuAnalyzer::analyze(){
  ...
  if(genChLeptons_->size()==3){
    ...
    if( genElectrons_->size()==2 && genMuons_->size()==1 
        ...
        && genElectrons_->at(0).pt()>5 && genElectrons_->at(1).pt()>5 && genMuons_->at(0).pt() > 5
        && -2.5<genElectrons_->at(0).eta()<2.5 && -2.5<genElectrons_->at(1).eta()<2.5 && -2.5<genMuons_->at(0).eta()<2.5 ){  // MODE 1 e+ e- mu nu
      ...
      if(isGen_mode1(genFourLepInvMass) == true){
      ...
      if(electrons->size()==2 && muons->size()==1
        ...
        if(isGen_mode1(genFourLepInvMass) == true && isRec_mode1(recFourLepTransverseMass/*, recElPairInvMass, recMu.pt()*/, recEl1.pt()+recEl2.pt()+recMu.pt()) == true){
          
          // -- Signal Efficiency -- //       
          theHistograms->fill("GEN_REC_signal_mode1", "Signal Efficiency (mode 1)", 3, 0., 3., 1, theWeight);
        }
        else if(isGen_mode1(genFourLepInvMass) == false && isRec_mode1(recFourLepTransverseMass/*, recElPairInvMass, recMu.pt()*/, recEl1.pt()+recEl2.pt()+recMu.pt()) == true){
          ...
          theHistograms->fill("not_GEN_REC_signal_mode1", "Background Efficiency (mode 1)", 3, 0., 3., 1, theWeight);
             ...
             && -2.5<genMuons_->at(0).eta()<2.5 && -2.5<genMuons_->at(1).eta()<2.5 && -2.5<genElectrons_->at(0).eta()<2.5 ){    // MODE 2 mu+ mu- e nu
      ...
      // ------------------------------ GEN LEVEL SIGNAL DEFINITION ----------------------------- //
      if(isGen_mode2(genFourLepInvMass) == true){
      ...
      if(muons->size()==2 && electrons->size()==1 
        ...
        // ------------------------------ REC LEVEL SIGNAL DEFINITION ----------------------------- //
        if(isGen_mode2(genFourLepInvMass) == true && isRec_mode2(recFourLepTransverseMass/*, recMuPairInvMass, recEl.pt()*/, recMu1.pt()+recMu2.pt()+recEl.pt()) == true){
          
          // -- Signal Efficiency -- //
          theHistograms->fill("GEN_REC_signal_mode2", "Signal Efficiency (mode 2)", 3, 0., 3., 1, theWeight);
        }
        else if(isGen_mode2(genFourLepInvMass) == false && isRec_mode2(recFourLepTransverseMass/*, recMuPairInvMass, recEl.pt()*/, recMu1.pt()+recMu2.pt()+recEl.pt()) == true){
      ...
      // ------------------- RECCOSTRUCTED EVENTS ----------------- //
    	if(electrons->size()==3 && muons->size()==0 && checkLeptonsCharge(electrons->at(0), electrons->at(1), electrons->at(2))==true){
      ...
      // ------------------- RECCOSTRUCTED EVENTS ----------------- //
    	if(muons->size()==3 && electrons->size()==0 && checkLeptonsCharge(muons->at(0), muons->at(1), muons->at(2))==true){
...
bool WlllnuAnalyzer::isRec_mode1(double var1/*, double var2, double var3*/, double var4){
  if(60.<var1<120./* && 4.<var2 && var2<12. && 10.<var3 && var3<20.*/ && 30<var4 && var4<70){
    return true;
  }
  else{
    return false;
