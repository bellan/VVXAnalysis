...
void WZZAnalyzer::genAnalyze()
{
  ...
  {
    if (abs(gen.id()) == 13)
    { // for muons: mc pdg id = 13 by notation
    ...
    if (abs(gen.id()) == 11)
    { // for electrons: mc pdg id = 11 by notation
  ...
  { // For the signal definition, the categories with the first bit on (i.e. bit0=true), are the only ones that have to be considered
    ...
    if (/*centralJets*/ jets->size() > 1)
    {
      ...
      foreach (const phys::Boson<phys::Particle> &gen, *genVBParticles)
      {
        ...
        {
          if (abs(gen.id()) == 23 && gen.mass() > 60 && gen.mass() < 120 && (gen.daughter(0)).pt() > 30 && (gen.daughter(1)).pt() > 30)
          { // for Z bosons: mc pdg id = 23 by notation
