...
echo "### Fitting freeze_datadr ###"
combine $rootpostfit -M MultiDimFit --snapshotName MultiDimFit $fit_options -n .freeze_datadr --freezeNuisanceGroups datadr || print_error "Fit freeze_datadr"

echo "### Fitting freeze_datadr_lumi ###"
combine $rootpostfit -M MultiDimFit --snapshotName MultiDimFit $fit_options -n .freeze_datadr_lumi --freezeNuisanceGroups datadr,lumi || print_error "Fit freeze_datadr_lumi"

echo "### Fitting freeze_datadr_lumi_theory ###"
combine $rootpostfit -M MultiDimFit --snapshotName MultiDimFit $fit_options -n .freeze_datadr_lumi_theory --freezeNuisanceGroups datadr,lumi,theory || print_error "Fit freeze_datadr_lumi_theory"
...
plot1DScan.py $(printf $rootpattern total) --main-label "Total Uncert." --others \
    $(printf $rootpattern freeze_datadr):"freeze datadr":4 \
