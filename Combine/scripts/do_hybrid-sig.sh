...
mkdir -p $workdir && cd $workdir || { echo "Unable to make dir $workdir" 1>&2 ; exit 2 ; }
...
for seed in $(seq 1 16); do
    ...
    echo "# seed $seed #"
    combine $gentoys_options -s $seed $card &> toys$seed.log &
done
wait || print_error "Generating toys"

echo "### Hadd-ing toys ###"
[ -f merged_toys.root ] || \
    hadd -f merged_toys.root higgsCombineTest.HybridNew.mH120.*.root || print_error "Hadd-ing toys" 

echo "### Significance (unblind: $unblind) ###"
echo "DEBUG: flags = $signif_options --toysFile=merged_toys.root"
if $unblind ; then
    combine $signif_options --toysFile=merged_toys.root $card > signif.log # || print_error "Observed signif"
    sed -n "/Significance/,//p" signif.log > significance.txt
else
    for expect in 0.16 0.5 0.84 ; do
	{
	    echo "DEBUG:         --expectedFromGrid=$expect"
	    combine $signif_options --expectedFromGrid=$expect $card --toysFile=merged_toys.root > signif_$expect.log
	    sed -n "/Significance/,//p" signif_$expect.log > significance_$expect.txt
	} &
    done
fi
wait
