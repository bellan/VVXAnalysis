...
public:
  ...
  WZGenFilterCategory(const ParameterSet& pset)
    ...
    , genJetsAK8Token_    (consumes<std::vector<reco::GenJet>  >(pset.getParameter<edm::InputTag>("GenJetsAK8")))
  {
    if(particleStatus_ != 1 && particleStatus_ != 3)
      edm::LogError("WZGenFilterCategory") << "WZGenFilterCategory: input for signal definition non available!";

    produces<int>();
    produces<std::vector<reco::GenParticle> >("vectorBosons");
    produces<std::vector<reco::GenParticle> >("genJets");
    produces<std::vector<reco::GenParticle> >("genJetsAK8");
...
void WZGenFilterCategory::beginJob() {}

bool WZGenFilterCategory::filter(Event & event, const EventSetup& eventSetup) { 
  ...
  case 1:{
    ...
    edm::Handle<std::vector<reco::GenJet> > genJetsAK8H;
  ...
  case 3:
	...
	if ( (id < 7 || id == 21) && idx > 5 ) { genJets.push_back(phys::convert(*p)); } // quarks and gluons // FIXME: AK8???
  ...
  if (particleStatus_ == 1) zzSignalTopology = zz::getSignalTopology(genLeptons, genJets, genJetsAK8);
...
DEFINE_FWK_MODULE(WZGenFilterCategory);
