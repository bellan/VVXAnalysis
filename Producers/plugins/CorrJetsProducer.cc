...
void CorrJetsProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
    ...
    auto corrJets = std::make_unique<pat::JetCollection>();
	...
	if (year_ == 2016 || year_ == 2017) 
	  if (jet.hasUserFloat("ak8PFJetsCHSPrunedMass"))
	    corrMass   = jet.userFloat("ak8PFJetsCHSPrunedMass") * jecAK8->getCorrection();
	  else if (jet.hasUserFloat("ak8PFJetsCHSValueMap:ak8PFJetsCHSPrunedMass"))
	    corrMass   = jet.userFloat("ak8PFJetsCHSValueMap:ak8PFJetsCHSPrunedMass") * jecAK8->getCorrection();
	  else
	    edm::LogError("CorrJEtsProducer") << "Do not know what to do with AK8 correction" << endl;
	else if (year_ == 2018) 
	  corrMass   = jet.userFloat("ak8PFJetsCHSValueMap:ak8PFJetsCHSPrunedMass") * jecAK8->getCorrection();
	  else
	    edm::LogError("CorrJEtsProducer") << "Do not know what to do with AK8 correction for " << year_ << endl;

        pat::Jet* cloneJet = jet.clone();
        cloneJet->addUserFloat("ak8PFJetsCHSCorrPrunedMass", corrMass );
        corrJets->push_back( *cloneJet );
    } 
    iEvent.put(std::move(corrJets), "corrJets");
}
