...
double FrixioneIsoCalculator::maxEnergyFraction(double delta) const{
  // Case nExponent_ != 1 is not handled
  // return epsilon_*(std::cos(1 - delta)/std::cos(1 - delta0_));
  static double cached_part = epsilon_/std::cos(1 - delta0_);
  return cached_part*std::cos(1 - delta);
}

bool FrixioneIsoCalculator::isIsolated(const reco::GenParticle* photon, const edm::View<reco::GenParticle>& genParticles_cand){
  cacheVector(genParticles_cand);
  return isIsolated(photon);
}

bool FrixioneIsoCalculator::isIsolated(const reco::GenParticle* photon){
  ...
  for(const reco::GenParticle* gp : cachedGenParticles_){
    ...
    double e_max = photon_pt*maxEnergyFraction(dr);
