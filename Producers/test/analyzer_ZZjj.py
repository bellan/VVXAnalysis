...
process.treePlanter = cms.EDAnalyzer("TreePlanter",
                                     channel = cms.untracked.string('LLLL'),
                                     ...
                                     ZZ4m         = cms.InputTag("MMMMCand"),
                                     ZZ4e         = cms.InputTag("EEEECand"),
                                     ZZ2e2m       = cms.InputTag("EEMMCand"),
...
process.WjjSequence = cms.Sequence(process.centralJets * process.bareWCand * process.WCand)#  * process.wPt)

# Use the same sequence as in H->ZZ
# process.Candidates += process.WjjSequence 


# This line needs to be modified in case of a run in the CRs
...
process.preselection = cms.Path(process.preSkimCounter*
                                (process.ZZFiltered * process.zzCounterFilter * process.disambiguatedJets +
                                 process.WjjSequence +
                                 process.jetPtFilter*process.jetCounterFilter)*
                                process.postSkimCounter)

#process.MoreCandidates = cms.Path()
